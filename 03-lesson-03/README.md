...
**Before you start** — this is the first lesson that *writes*. Open SQL Explorer
and select `chinook.db` from the database dropdown:

SQL Explorer works on an in-memory copy. Click **Load** again whenever you need
to reset the lesson; the GitHub database stays untouched. Immediately after
each load, run `PRAGMA foreign_keys = ON;` so foreign-key behavior matches
